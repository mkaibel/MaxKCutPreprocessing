...
for small in {True, False}:
    ...
    if small:
        plt.title('Average runtime by k on Small Instances')
    else:
        plt.title('Average runtime by k on Large Instances')
    plt.xlabel('k')
    #plt.ylim(0, None)
    plt.ylabel('Average runtime (ms)')
    plt.yscale('log')
    plt.legend(title='Average Runtime by Preprocessor Config')
    plt.grid(True)
    # plt.show()
    ...
    if small:
        plt.savefig(f'plots/ablation_average_runtime_by_k_small.pdf', format='pdf', dpi=300, bbox_inches='tight')
    else:
        plt.savefig(f'plots/ablation_average_runtime_by_k_large.pdf', format='pdf', dpi=300, bbox_inches='tight')
